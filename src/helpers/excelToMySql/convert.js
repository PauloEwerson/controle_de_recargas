...
const rowsFromExcel = async function (filePath, callback) {
  ...
  if (extension === 'csv' || extension === 'CSV') {
    fs.readFile(filePath, 'utf8', (error, sdata) => {
      if (error) {
        callback(error);
      } else {
        csv({
    ...
    readExcel(fs.createReadStream(filePath)).then((rows) => {
      callback(null, rows);
    });
  }
};
...
exports.convertToFile = function (data, options, callback) {
  ...
  return new Promise((async (resolve, reject) => {
    rowsFromExcel(data.path, (error, rows) => {
      if (error) {
        reject(error);
...
exports.covertToMYSQL = function (data, options, callback) {
  ...
  return new Promise((async (resolve, reject) => {
    ...
    const connection = data.connection || mysql.createConnection({
      host: data.host,
      user: data.user,
      password: data.pass,
      ...
      if (error) {
        ...
        rowsFromExcel(data.path, (error, rows) => {
          if (error) {
            ...
            connection.query(`create database if not exists ${data.db};use ${data.db};create table if not exists ${data.table} (${tableString})`, (error) => {
              if (options.verbose) {
              ...
              if (error) {
                ...
                // Create the string first and then insert the full data at once.
                const insertString = createInsertTable(options, rows, reject, callback);
                connection.query(`insert into ${data.table} values${insertString}`, (error, results) => {
                  if (options.verbose) {
